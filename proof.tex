...
\displaystyle\sum^{G-1}_{b=0}2^bM_{k-1}(g_b)&\oth\cr}\right.\eqno{(1)}$$
...
$$M_3(f_3(a))=\sum^a_{b=0}2^bM_2(f_2(a-b))=h_3(a)2^a-1$$ for some
$h_3(a)$.  We then see that the general form of the perfect solution
to $M_k$ is,
...
And as we expand the function $h$ more and more, it ``sheds'' off an
alternating series that is dependant on $f$ because of the nested sums
each term of it was passed through.  Finally, when $h$ has reached its
edge condition, we are left with a pure nested sum, which is exactly
